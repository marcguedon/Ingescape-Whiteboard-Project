...
from Puissance4_Controller import *

port = 5670
agent_name = "Puissance4_Controller"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Puissance4_Controller)
...
# services
def Token_Placed_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Puissance4_Controller)
        agent_object.Token_Placed(sender_agent_name, sender_agent_uuid)
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    agent = Puissance4_Controller()

    igs.observe_agent_events(on_agent_event_callback, agent)
    igs.observe_freeze(on_freeze_callback, agent)

    igs.service_init("token_placed", Token_Placed_callback, agent)
